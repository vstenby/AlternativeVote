...
def print_results(running, votecount, ncount, nround=False):
    votepct = votecount/np.sum(votecount)*100
    
    s = 'The votes have been counted. These are the results:'
    if nround:
        print(f'Round {nround} | Count {ncount}'.center(len(s)))
    else:
...
def main():
    ...
    while True:
        ...
        #Except those, who have won already.
        running = running[np.invert(np.isin(running,winners))]
        
        if len(running) == 0:
            #No more people running.
            break
        ...
        while True:
            ...
            if np.all(votecount == votecount[0]) and len(votecount) > 1:
                raise ValueError('Draw.')
           
            #Count the percentages
            votepct = votecount/np.sum(votecount)*100
            
            print_results(running, votecount, i, k)
    ...
    print('')
    print('The order of the candidates is:')
